...
>(({ className, children, isMultiSelect, selectedValues, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ...
    className={cn(
      ...
      isMultiSelect ? "[&>span]:line-clamp-2" : "[&>span]:line-clamp-1",
      className,
    )}
    {...props}
  >
    {isMultiSelect && selectedValues && selectedValues.length > 0 ? (
      <div className="flex flex-wrap gap-1 max-w-full py-1">
        {selectedValues.slice(0, 3).map((value) => (
          <span key={value} className="inline-flex items-center rounded-md bg-muted px-2 py-1 text-xs font-medium">
            {value}
          </span>
        ))}
